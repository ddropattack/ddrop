...
int main(int argc, char **argv)
{
    ...
    uint64_t tdcs_pa = 0;
    printf("Enter GPA mapped to own TDCS: ");
    ...
    //tdcs_pa += 128; // Attributes are at offset 128
    ...
    printf("Written to TDCS. This TD should now be a debug TD\n\n");
    memset(buf, 0, ALLOC_SIZE);
    if( memcpy_frompa(&(original_attrs.raw), tdcs_pa, sizeof(td_param_attributes_t), &stats, true) ) {
        goto error;
    }

    printf("Read TDCS Attributes from memory (%zu bytes):\n", sizeof(td_param_attributes_t));
    hexdump((uint8_t *)&(original_attrs.raw), sizeof(td_param_attributes_t));
